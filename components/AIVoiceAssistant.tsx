...
interface AIVoiceAssistantProps {
  twilioNumber: string;
  onMakeCall: (prompt: string, phoneNumber: string, voice?: string) => Promise<any>;
}

export default function AIVoiceAssistant({ twilioNumber, onMakeCall }: AIVoiceAssistantProps) {
  const [prompt, setPrompt] = useState('');
  const [phoneNumber, setPhoneNumber] = useState('');
  const [isProcessing, setIsProcessing] = useState(false);
  const [callStatus, setCallStatus] = useState<'idle' | 'scheduled' | 'in-progress' | 'completed' | 'failed'>('idle');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await onMakeCall(prompt, formattedPhoneNumber, selectedVoice);
      ...
      const pollCall = async () => {
        try {
          ...
          pollCount++;
          console.log(`Polling call status (attempt ${pollCount}/${maxPolls})...`);
          
          const statusResult = await fetch(`/api/bland-ai?callId=${result.callId}`).then(r => r.json());
          console.log('Poll result:', statusResult);
          
          if (statusResult.status === 'completed' || statusResult.status === 'failed') {
            setCallStatus(statusResult.status === 'completed' ? 'completed' : 'failed');
            ...
            if (statusResult.status === 'completed' && (!statusResult.recording || !statusResult.transcript)) {
              setTimeout(async () => {
                try {
                  const finalResult = await fetch(`/api/bland-ai?callId=${result.callId}`).then(r => r.json());
                  if (finalResult.recording || finalResult.transcript) {
                    setCallResult(prev => ({
                      ...prev,
                      recording: finalResult.recording || prev.recording,
                      transcript: finalResult.transcript || prev.transcript,
                      summary: finalResult.summary || prev.summary
                    }));
                  }
                } catch (finalError) {
      ...
      const errorMessage = err instanceof Error ? err.message : 
        typeof err === 'object' && err !== null && 'details' in err ? 
          String(err.details) : 'Failed to initiate call. Please try again.';
  ...
  const handleManualRefresh = async () => {
    ...
    try {
      setIsProcessing(true);
      const refreshResult = await fetch(`/api/bland-ai?callId=${callResult.callId}`).then(r => r.json());
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      ...
      <p className="text-gray-600 mb-4">
        The AI will call the number you specify using BlandAI's phone system with the "Cailee" voice and follow your instructions.
      </p>
      ...
      <form onSubmit={handleSubmit} className="mb-6">
        ...
        <button
          type="submit"
          disabled={isProcessing || callStatus === 'in-progress' || !prompt.trim() || !phoneNumber.trim()}
          className={`w-full py-2 px-4 rounded-md text-white font-medium ${
            isProcessing || callStatus === 'in-progress' ? 'bg-blue-400' : 'bg-blue-500 hover:bg-blue-600'
          }`}
        >
          {isProcessing ? (
        ...
        <div className="border-t border-gray-200 pt-4">
          <div className="flex items-center mb-3">
            ...
            <span className={`ml-2 px-2 py-1 text-xs rounded-full ${
              callStatus === 'scheduled' ? 'bg-yellow-100 text-yellow-800' : 
              callStatus === 'in-progress' ? 'bg-blue-100 text-blue-800' : 
            ...
            <div className="space-y-4">
              {callResult.summary && (
                <div>
                  ...
                  <p className="text-gray-600 bg-gray-50 p-3 rounded">{callResult.summary}</p>
                </div>
              )}
              
              {callResult.transcript && callResult.transcript !== 'No transcript available' ? (
                <div>
                ...
                <div>
                  ...
                  <p className="text-gray-500 italic bg-gray-50 p-3 rounded">
                    Transcript not available. BlandAI may still be processing the call recording.
                  </p>
                </div>
              )}
              ...
              {callResult.recording ? (
                ...
                <div>
                  <h4 className="font-medium text-gray-700 mb-1">Recording</h4>
                  <p className="text-gray-500 italic bg-gray-50 p-3 rounded">
                    Recording not available. BlandAI may still be processing the call recording.
                  </p>
                </div>
              )}
              
              <div className="mt-2 text-sm text-gray-500">
                <p>Note: Recordings and transcripts may take a few minutes to process after the call completes.</p>
              </div>
            </div>
          )}
